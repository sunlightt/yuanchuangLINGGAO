...
function slidebanner(option){
    ...
    $(option.contain1).css({'width':100+'%'});
    $(option.contain2).find('li').width($(option.contain1).width());
    $(option.contain2).find('li img').width(100+'%');
    $(option.contain1).height($(option.contain2).find('li').eq(0).get(0).offsetHeight);
    ...
    function autoPlay(){
        ...
        timer=setInterval(function(){
            ...
            $(option.contain2).stop().animate({'left':-num*$(option.contain1).width()},2000,'swing');
            ...
            $('.slide_img .banner_img li').eq(num).addClass('active').siblings().removeClass('active');
    ...
    $(option.btn).children().each(function(i){
        ...
        $(this).on('mouseover',function(){
            ...
            $(option.btn).children('li').eq($(this).index()).addClass('active').siblings().removeClass('active');

            $('.slide_img .banner_img li').eq($(this).index()).addClass('active').siblings().removeClass('active');
            num=$(this).index();
        });
    });
    ...
    $(option.sldir[0]).click(function(){
        ...
        $(option.contain2).stop().animate({left:-num*$(option.contain1).width()},1000);
        $(option.btn).children().eq(num).addClass('active').siblings().removeClass('active');
